...
//Snake
  void drawSnake() {
    ...
    for (int i = 0; i < length; i++) {
        ...
        double len=(snake[i].x+snake[i].x + (CELL_SIZE-3))/2;
        double width=(snake[i].y+snake[i].y+ (CELL_SIZE-3))/2;





        if((len>=food.x)&&(width>=food.y)){
            length++;
          food.x+=25;
          food.y+=25;
...
//Box
 ...
 //main
   int main() {
    ...
    gd = DETECT, gm;
    initgraph(&gd, &gm, "C:\\Turboc3\\BGI");
    time_t startTime1, currentTime1;
    time(&startTime1);
    food.x=125;
    food.y=125;
     for (int i = 0; i < length; i++) {
    ...
    //Time
        time(&currentTime1);
         double Time = difftime(currentTime1, startTime1);
   ...
   if(records[1].score > records[2].score){
     printf("Winner:%s",records[1].playerName);
   }
   else if(records[1].score==records[2].score) {
    printf("Draw");
   }
   else printf("Winner:%s",records[2].playerName);
